...
def exp_gauss_dist(x, a, mu, sigma, lamb, b):
    if lamb * sigma <= 0:
        return x*0
    else:
        err = (mu + lamb*sigma**2-x) / (np.sqrt(2)*sigma)
        pow = lamb / 2 * (2*mu + lamb*sigma**2 - 2*x)
        return np.nan_to_num(lamb / 2 * a * np.exp(pow) * erfc(err) + b, posinf=0, neginf=0)

# parameter gradient
def exp_gauss_dist_jac(x, a, mu, sigma, lamb, b):
    pow = lamb / 2 * (2*mu + lamb*sigma**2 - 2*x)
    f0 = exp_gauss_dist(x, a, mu, sigma, lamb, 0)
    a_deriv = f0 / a
    mu_deriv = f0 * lamb - lamb * gauss_dist(x, a, mu, sigma, 0)
    sigma_deriv = f0 * lamb**2 + (lamb * (mu - x)/sigma - sigma*lamb**2) * gauss_dist(x, a, mu, sigma, 0)
    lamb_deriv = f0 * (mu - x + 1 + lamb*sigma**2) - lamb*sigma**2*gauss_dist(x,a,mu+lamb*sigma**2,sigma, 0) * np.exp(pow)
    if isinstance(x, np.ndarray) != 1:
        b_deriv = 1
    else:
        b_deriv = np.ones(len(x))
    return np.nan_to_num(np.array([a_deriv, mu_deriv, sigma_deriv, lamb_deriv, b_deriv]), posinf=0, neginf=0)

# Hessian
def exp_gauss_dist_hess_matrix(x, a, mu, sigma, lamb, b):
    pow = lamb / 2 * (2*mu + lamb*sigma**2 - 2*x)
    f0 = exp_gauss_dist(x, a, mu, sigma, lamb, 0)
    jacob = exp_gauss_dist_jac(x, a, mu, sigma, lamb, b)
    jacob_gauss = gauss_dist_jac(x, a, mu, sigma, b)
    jacob_gauss1 = gauss_dist_jac(x, a, mu+lamb*sigma**2, sigma, b)
    g0 = gauss_dist(x, a, mu, sigma, 0)
    g1 = gauss_dist(x, a, mu+lamb*sigma**2, sigma, 0)

    grad_deriv_a = np.insert(np.insert(jacob[1:-1] / a, 0, 0), 4, 0)

    grad_deriv_mu = np.insert(np.insert([lamb**2*f0 + (lamb*(mu-x)/sigma**2 + lamb**2*(1-2*np.pi)) * g0,
                                         lamb*(jacob[2]-jacob_gauss[2]), jacob[1]/lamb+lamb*jacob[3]]
                              , 0, jacob[1] / a), 4, 0)

    grad_deriv_sig = np.insert(np.insert([lamb**2*jacob[1] + lamb/sigma * g0 + (lamb * (mu - x)/sigma - sigma*lamb**2) * jacob_gauss[1],
                                          lamb**2*jacob[2] - (lamb*(mu-x)/sigma**2+lamb**2)*g0 + (lamb * (mu - x)/sigma - sigma*lamb**2) * jacob_gauss[2],
                                          2*lamb*f0 + lamb**2*jacob[3] + ((mu-x)/sigma-2*sigma*lamb)*g0]
                               , 0, jacob[2] / a), 4, 0)

    grad_deriv_lamb = np.insert(np.insert([f0 - lamb*sigma**2 * (jacob_gauss1[1] + lamb*g1)*np.exp(pow),
                                           2*sigma*lamb*f0 - 2*lamb**2*sigma**3 * (jacob_gauss1[2] + (1 + 1/(lamb*sigma**2)) * g1)*np.exp(pow),
                                           sigma**2*f0 - lamb*sigma**2 * (sigma**2*jacob_gauss1[1] +(mu-x+lamb*sigma*2+2/sigma)*g1)*np.exp(pow)]
                               , 0, jacob[3] / a), 4, 0)

    grad_deriv_b = np.array([0, 0, 0, 0, 1])
    return np.array([grad_deriv_a, grad_deriv_mu,
                     grad_deriv_sig, grad_deriv_lamb, grad_deriv_b])

def exp_gauss_dist_hess(x, a, mu, sigma, lamb, b):
    if isinstance(x, np.ndarray):
        ret = exp_gauss_dist_hess_matrix(x[0], a, mu, sigma, lamb, b)
        for i in range(1, len(x)):
            ret = stack(ret, exp_gauss_dist_hess_matrix(x[i], a, mu, sigma, lamb, b))
        return np.array(ret)
    else:
        return np.nan_to_num(exp_gauss_dist_hess_matrix(x, a, mu, sigma, lamb, b), posinf=0, neginf=0)

# Two Exponentially modified Gaussian distribution

def two_exp_gauss_dist(x, a1, a2, mu, sigma, lamb1, lamb2, b):
    if lamb1 * sigma <= 0 or lamb2 * sigma <= 0 or lamb1 >= lamb2:
        return x*0
    else:
        return exp_gauss_dist(x,a1,mu,sigma,lamb1,0) + exp_gauss_dist(x,a2,mu,sigma,lamb2,0) + b

# parameter gradient
def two_exp_gauss_dist_jac(x, a1, a2, mu, sigma, lamb1, lamb2, b):
    jacob1 = exp_gauss_dist_jac(x,a1,mu,sigma,lamb1,b)
    jacob2 = exp_gauss_dist_jac(x,a2,mu,sigma,lamb2,b)
    
    a1_deriv = jacob1[0]
    a2_deriv = jacob2[0]
    mu_deriv = jacob1[1] + jacob2[1]
    sigma_deriv = jacob1[2] + jacob2[1]
    ...
    if isinstance(x, np.ndarray) != 1:
        ...
        b_deriv = np.ones(len(x))
    return np.nan_to_num(np.array([a1_deriv,a2_deriv, mu_deriv, sigma_deriv, lamb1_deriv, lamb2_deriv, b_deriv]), posinf=0, neginf=0)

# Hessian
def two_exp_gauss_dist_hess_matrix(x, a1, a2, mu, sigma, lamb1, lamb2, b):
    hess1 = exp_gauss_dist_hess_matrix(x, a1, mu, sigma, lamb1, b)
    hess2 = exp_gauss_dist_hess_matrix(x, a2, mu, sigma, lamb2, b)

    grad_deriv_a1 = np.insert(np.insert(hess1[0], 1, 0), 5, 0)

    grad_deriv_a2 = np.insert(np.insert(hess2[0], 0, 0), 4, 0)

    grad_deriv_mu = np.insert(np.insert(hess1[1], 1, 0), 5, 0) + np.insert(np.insert(hess2[1], 0, 0), 4, 0)

    grad_deriv_sig = np.insert(np.insert(hess1[2], 1, 0), 5, 0) + np.insert(np.insert(hess2[2], 0, 0), 4, 0)

    grad_deriv_lamb1 = np.insert(np.insert(hess1[3], 1, 0), 5, 0)
    
    grad_deriv_lamb2 = np.insert(np.insert(hess2[3], 0, 0), 4, 0)

    grad_deriv_b = np.array([0, 0, 0, 0, 0, 0, 1])
    return np.array([grad_deriv_a1, grad_deriv_a2, grad_deriv_mu,
                     grad_deriv_sig, grad_deriv_lamb1, grad_deriv_lamb2, grad_deriv_b])

def two_exp_gauss_dist_hess(x, a1, a2, mu, sigma, lamb1, lamb2, b):
    if isinstance(x, np.ndarray):
        ret = two_exp_gauss_dist_hess_matrix(x[0], a1, a2, mu, sigma, lamb1, lamb2, b)
        for i in range(1, len(x)):
            ret = stack(ret, two_exp_gauss_dist_hess_matrix(x[i], a1, a2, mu, sigma, lamb1, lamb2, b))
        return np.array(ret)
    else:
...
class FittingRoutine:
    ...
    covx : ndarray, shape (n,n), optional
